...
class List {
 ...
 public:
  ...
  Position findData(const T&, int(const T&, const T&));
...
template <class T>
typename List<T>::Position List<T>::findData(const T& e, int cmp(const T&, const T&)) {
  Position aux(this->anchor);

  while (aux != nullptr && cmp(aux->getData(), e) != 0) {
    aux = aux->getNext();
  }
